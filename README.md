...
Главное: в таблице поддерживаемых устройств OpenWrt должна быть именно твоя модель и ревизия (там же лежат прошивки).
...
2. В **Управление → Параметры системы → Изменить набор компонентов** включи компонент **WireGuard VPN**.[^2][^1]
...
- Репозиторий RockBlack‑VPN: https://github.com/RockBlack-VPN/ip-address – там `.bat` и списки IP для YouTube, Telegram, OpenAI и т.п..[^6][^7]
- Для AmneziaWG/keenetic есть ещё готовые наборы у BlackTemple и других провайдеров (часто дают прям ссылку «файлы маршрутизации для Keenetic» ).[^8]
...
3. В поле **Интерфейс** выбери **`de`** (имя твоего AmneziaWG‑подключения, не Ethernet!).[^9][^10]
...
2. По инструкции Amnezia для OpenWrt ставишь пакеты:[^4][^5]
```sh
opkg update
opkg install kmod-amneziawg amneziawg-tools luci-proto-wireguard
```

(На свежих сборках пакет может называться `kmod-awg` – это в репо `amneziawg-openwrt`.)[^3][^5]
...
6. Для ASC‑параметров AmneziaWG на OpenWrt обычно работает через отдельный userspace демон (amneziawg‑tools), который подхватывает параметры из конфига – точная связка описана в их docs.[^4][^5]
...
pbr через resolver sets сам создаст ipset’ы и будет направлять нужные домены в AmneziaWG‑туннель.[^11][^12]
...
- в `dnsmasq` основным оставь 1.1.1.1/8.8.8.8, pbr сам привяжет «правильные» запросы к `awg_de`.[^12][^11]
